...
function [] = generate_data_client(dataset_name, client_idx, path_source, path_target, frame_count, lts_count, frame_len, lts_len, ant_rows, iao)
    ...
    [~, elevation] = utils.get_ground_truth(getfield(mimo_run_data, 'client_coord'));
    ...
    for frame_idx = 1:frame_count
        for col_idx = 1:4
            try
                % Prepare indexes of antennas in 1st column (w/ 1-based indexing)
                ant_col_idx = ant_rows(:, col_idx) + 1;
                ...
                client_lts_indexes = client_lts_positions(frame_idx + 1, ant_col_idx) + 1;
        
                % Extract samples for each of the antennas we need
                client_ant1 = client_frame(ant_col_idx(1), :)';
                client_ant2 = client_frame(ant_col_idx(2), :)';
                client_ant3 = client_frame(ant_col_idx(3), :)';
                client_ant4 = client_frame(ant_col_idx(4), :)';
                client_ant5 = client_frame(ant_col_idx(5), :)';
                client_ant6 = client_frame(ant_col_idx(6), :)';
                ...
                client_ant5_corr = utils.run_doppler_calibration(client_ant5, frame_len, client_lts_indexes(5), 'Antenna 5', lts_len, false);
                client_ant6_corr = utils.run_doppler_calibration(client_ant6, frame_len, client_lts_indexes(6), 'Antenna 6', lts_len, false);
                ...
                client_ant2_final = client_ant2_corr.*exp(-1j * iao(client_idx, col_idx, 1));
                client_ant3_final = client_ant3_corr.*exp(-1j * iao(client_idx, col_idx, 2));
                client_ant4_final = client_ant4_corr.*exp(-1j * iao(client_idx, col_idx, 3));
                client_ant5_final = client_ant5_corr.*exp(-1j * iao(client_idx, col_idx, 4));
                client_ant6_final = client_ant6_corr.*exp(-1j * iao(client_idx, col_idx, 5));
        
                % Prepare MUSIC input array (1D MUSIC can be run on this matrix)
                client_X = zeros(6, lts_len * lts_count); 
                ...
                client_X(5, :) = client_ant5_final(client_lts_indexes(5) : client_lts_indexes(5) + lts_len * lts_count - 1)';
                client_X(6, :) = client_ant6_final(client_lts_indexes(6) : client_lts_indexes(6) + lts_len * lts_count - 1)';
    
                % Prepare output (take only 1st sample from the array)
                output_samples(1, col_idx, :, frame_idx) = reshape(client_X(1, :), [128, 1]);
                output_samples(2, col_idx, :, frame_idx) = reshape(client_X(2, :), [128, 1]);
                output_samples(3, col_idx, :, frame_idx) = reshape(client_X(3, :), [128, 1]);
                output_samples(4, col_idx, :, frame_idx) = reshape(client_X(4, :), [128, 1]);
                output_samples(5, col_idx, :, frame_idx) = reshape(client_X(5, :), [128, 1]);
                output_samples(6, col_idx, :, frame_idx) = reshape(client_X(6, :), [128, 1]);
            catch error
                % disp(strcat('Problems with LTS indexing. Column #', int2str(col_idx)));
    
                % Prepare output (take only 1st sample from the array)
                output_samples(1, col_idx, :, frame_idx) = NaN;
                output_samples(2, col_idx, :, frame_idx) = NaN;
                output_samples(3, col_idx, :, frame_idx) = NaN;
                output_samples(4, col_idx, :, frame_idx) = NaN;
                output_samples(5, col_idx, :, frame_idx) = NaN;
                output_samples(6, col_idx, :, frame_idx) = NaN;
            end
        end
    end
    
    % 3. Save output array for further processing
    save_filename = strcat(path_target, 'client_x', num2str(client_idx), '_for_elev.mat');
    for i = 1:frame_count
        varname = strcat('output_samples_frame_', num2str(i));
        S.(varname) = squeeze(output_samples(:, :, :, i));
    end
    S.output_elev = elevation;
...
function [] = generate_data_reference(dataset_name, client_idx, path_source, path_target, frame_count, lts_count, frame_len, lts_len, ant_rows)
    % 1. Load original dataset
    mimo_run_data = load(strcat(path_source, dataset_name, '.mat'));
    [~, elevation] = utils.get_ground_truth(getfield(mimo_run_data, 'ref_coord'));
    ...
    for frame_idx = 1:frame_count
        for col_idx = 1:4
            try
                ...
                lts_indexes = reference_lts_positions(frame_idx + 1, ant_col_idx) + 1;
            
                % Extract samples for each of the antennas we need
                ant1 = frame(ant_col_idx(1), :)';
                ant2 = frame(ant_col_idx(2), :)';
                ant3 = frame(ant_col_idx(3), :)';
                ant4 = frame(ant_col_idx(4), :)';
                ant5 = frame(ant_col_idx(5), :)';
                ant6 = frame(ant_col_idx(6), :)';
                ...
                ant5_corr = utils.run_doppler_calibration(ant5, frame_len, lts_indexes(5), 'Antenna 5', lts_len, false);
                ant6_corr = utils.run_doppler_calibration(ant6, frame_len, lts_indexes(6), 'Antenna 6', lts_len, false);
            
                % Correct phases across antennas using lts_count LTS symbols
                [ant1_final, ant2_final, ant3_final, ant4_final, ant5_final, ant6_final, ~, ~, ~, ~, ~] = utils.run_interant_calibration6( ...
                    lts_indexes, ant1_corr, ant2_corr, ant3_corr, ant4_corr, ant5_corr, ant6_corr, lts_len, lts_count);
    
                % Run MUSIC on these four antennas
                X = zeros(6, lts_len * lts_count); 
                ...
                X(5, :) = ant5_final(lts_indexes(5) : lts_indexes(5) + lts_len * lts_count - 1)';
                X(6, :) = ant6_final(lts_indexes(6) : lts_indexes(6) + lts_len * lts_count - 1)';
    
                % Prepare output (take only 1st sample from the array)
                output_samples(1, col_idx, :, frame_idx) = reshape(X(1, :), [128, 1]);
                output_samples(2, col_idx, :, frame_idx) = reshape(X(2, :), [128, 1]);
                output_samples(3, col_idx, :, frame_idx) = reshape(X(3, :), [128, 1]);
                output_samples(4, col_idx, :, frame_idx) = reshape(X(4, :), [128, 1]);
                output_samples(5, col_idx, :, frame_idx) = reshape(X(5, :), [128, 1]);
                output_samples(6, col_idx, :, frame_idx) = reshape(X(6, :), [128, 1]);
            catch error
                % disp(strcat('Problems with LTS indexing. Row #', int2str(col_idx)));
    ...
    % 3. Save output array for further processing
    save_filename = strcat(path_target, 'reference_x', num2str(client_idx), '_for_elev.mat');
    for i = 1:frame_count
        varname = strcat('output_samples_frame_', num2str(i));
        S.(varname) = squeeze(output_samples(:, :, :, i));
    end
