...
% 'data'. The file name is 'iao_cols.mat'.
...
frame_antenna_offsets = zeros(length(MIMO_DATASETS), FRAME_COUNT, 4, 5);
    ...
    for frame_idx = 1:FRAME_COUNT
        ...
        for col_idx = 1:4
            try
                ...
                [ant1_final, ant2_final, ant3_final, ant4_final, ant5_final, ant6_final, w_ant12, w_ant13, w_ant14, w_ant15, w_ant16] = utils.run_interant_calibration6( ...
                    lts_indexes, ant1_corr, ant2_corr, ant3_corr, ant4_corr, ant5_corr, ant6_corr, LTS_LEN, LTS_COUNT);
                
                % Record inter-antenna offsets
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 1) = w_ant12;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 2) = w_ant13;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 3) = w_ant14;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 4) = w_ant15;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 5) = w_ant16;
            catch error
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 1) = -1;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 2) = -1;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 3) = -1;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 4) = -1;
                frame_antenna_offsets(file_idx, frame_idx, col_idx, 5) = -1;
            end
        end
    end
end
...
median_offsets_final = zeros(length(MIMO_DATASETS), 4, 5);

for col_idx = 1:4
    f = figure('Name', strcat('Inter-antenna Phase Offsets. LTS: ', num2str(LTS_COUNT), '. COL: ', num2str(col_idx)), 'NumberTitle', 'off');
    f.Position = [100 100 2000 800];
    for i = 1:size(frame_antenna_offsets, 1)
        % Calculate average offsets for each antenna pair
        offset_12 = median(unwrap(frame_antenna_offsets(i, :, col_idx, 1)));
        offset_13 = median(unwrap(frame_antenna_offsets(i, :, col_idx, 2)));
        offset_14 = median(unwrap(frame_antenna_offsets(i, :, col_idx, 3)));
        offset_15 = median(unwrap(frame_antenna_offsets(i, :, col_idx, 4)));
        offset_16 = median(unwrap(frame_antenna_offsets(i, :, col_idx, 5)));
    
        % Plot everything
        subplot(2, 4, i)
        plot(1:FRAME_COUNT, unwrap(frame_antenna_offsets(i, :, col_idx, 1)), '-red', 'DisplayName', 'Antenna 1-2');
        hold on
        plot(1:FRAME_COUNT, unwrap(frame_antenna_offsets(i, :, col_idx, 2)), '-green', 'DisplayName', 'Antenna 1-3');
        plot(1:FRAME_COUNT, unwrap(frame_antenna_offsets(i, :, col_idx, 3)), '-blue','DisplayName', 'Antenna 1-4');
        plot(1:FRAME_COUNT, unwrap(frame_antenna_offsets(i, :, col_idx, 4)), '-blue','DisplayName', 'Antenna 1-5');
        plot(1:FRAME_COUNT, unwrap(frame_antenna_offsets(i, :, col_idx, 5)), '-blue','DisplayName', 'Antenna 1-6');
        ...
        plot([1, FRAME_COUNT-1], [offset_15, offset_15], 'DisplayName', 'Antenna 1-5 (median)')
        plot([1, FRAME_COUNT-1], [offset_16, offset_16], 'DisplayName', 'Antenna 1-6 (median)')
        hold off
        legend
        grid on
        xlabel('Frame Index');
        ...
        % Save median offsets
        median_offsets_final(i, col_idx, 1) = offset_12;
        median_offsets_final(i, col_idx, 2) = offset_13;
        median_offsets_final(i, col_idx, 3) = offset_14;
        median_offsets_final(i, col_idx, 4) = offset_15;
        median_offsets_final(i, col_idx, 5) = offset_16;
    end
end
