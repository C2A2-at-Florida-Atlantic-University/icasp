...
for c = clients 
    ...
    true_aoa_client = double(load(work_space_client).('output_elev')); %convert single to double 
    ...
    for f = 1:19
        ...
        for s = 1:128
            ...
            [~,col_with_NaN] = find(NaN_matrix == 1);

            %removes this row/rows from data_matrix 
            data_matrix(:,col_with_NaN) = [];

            averaged_col = mean(data_matrix,2);

            [l1_final_aoa,l1_final_series,l1_final_max_magnitude,l1_windowWithMinMSE,l1_optimalKforWindow] = MSSA_L1_findOptimalWandK_filtering_onetime_max_magnitude(averaged_col,num_antennas,true_aoa_client,2,5,1,'s',step);
            [svd_final_aoa,svd_final_series,svd_final_max_magnitude,svd_windowWithMinMSE,svd_optimalKforWindow] = MSSA_SVD_findOptimalWandK_filtering_onetime_max_magnitude(averaged_col,num_antennas,true_aoa_client,2,5,1,'s',step);
...
for r = ref 
    ...
    true_aoa_ref = double(load(work_space_ref).('output_elev')); %convert single to double 
    ...
    for f = 1:19
        ...
        for s = 1:128
            ...
            [l1_final_aoa,l1_final_series,l1_final_max_magnitude,l1_windowWithMinMSE,l1_optimalKforWindow] = MSSA_L1_findOptimalWandK_filtering_onetime_max_magnitude(averaged_col,num_antennas,true_aoa_ref,2,5,1,'s',step);
            [svd_final_aoa,svd_final_series,svd_final_max_magnitude,svd_windowWithMinMSE,svd_optimalKforWindow] = MSSA_SVD_findOptimalWandK_filtering_onetime_max_magnitude(averaged_col,num_antennas,true_aoa_ref,2,5,1,'s',step);
