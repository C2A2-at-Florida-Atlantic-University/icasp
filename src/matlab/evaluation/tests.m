...
doa_true_angles_elev = [-50, -49.1, -48.1, -48.1, -50.22, -43.07, -34.16, -29.25, -85.67];
...
doa_threshold = 10; % threshold in degrees we use to pick samples
...
function [] = full_test_real_elev(path_source, lambda, Nr, N, doa_threshold)
    ...
    signal_real_elev_filtered = filter_signal(real_signal_elev, doa_true_angles_elev, Nr, doa_threshold, 5);
...
function [] = full_test_real_az(path_source, lambda, Nr, N, doa_threshold)
    ...
    % 2. Filter samples in each client & frame which are below given DoA threshold
    test_signal_sim_az_filtered = filter_signal(real_signal_az, doa_true_angles_az, Nr, doa_threshold, 3);
...
function [] = full_test_sim_elev(doa_true_angles_elev, d_col, T, lambda, Nr, N, SNR, rand_seed, doa_threshold)
    ...
    test_signal_sim_elev_filtered = filter_signal(sim_signal_elev, doa_true_angles_elev, Nr, doa_threshold, 5);
...
function [] = full_test_sim_az(doa_true_angles_az, d_row, T, lambda, Nr, N, SNR, rand_seed, doa_threshold)
    ...
    % 2. Filter samples in each client & frame which are below given DoA threshold
    % Create array of cells of a size (M x N), where M - # of clients, N - # of frames
    test_signal_sim_az_filtered = filter_signal(sim_signal_az, doa_true_angles_az, Nr, doa_threshold, 3);
    ...
    rmse_hankel_az_full = evaluate_hankel(doa_true_angles_az, signal_sim_az_full, Nr, N, 3);
...
function [signal_filtered] = filter_signal(signal, true_doa, Nr, doa_threshold, upper)
    ...
    for client_idx = 1:size(signal_filtered, 1)
        ...
        for frame_idx = 1:size(signal_filtered, 2)
            ...
            frame_good_samples = find_frame_good_samples_hankel(client_true_doa, client_samples, Nr, doa_threshold, client_idx, false, upper);
...
function [sample_good_idx] = find_frame_good_samples_hankel(true_doa, frame_samples, Nr, doa_threshold, client_idx, plot_errors, upper)
    frame_errors = zeros(128, 1);
    for sample_idx = 1:size(frame_errors, 1)
        X = squeeze(frame_samples(:, sample_idx));

        [svd_final_doa, ~, ~, ~, ~] = MSSA_SVD_findOptimalWandK_filtering_onetime_max_magnitude(X, Nr, true_doa, 2, upper, 1, 's', 1);
...
function [doa_rmse] = evaluate_hankel(doa_true_angles, test_signal_sim, Nr, N, upper)
    ...
    for doa_idx = 1:length(doa_true_angles) 
        ...
        for frame_idx = 1:N
            ...
            for sample_idx = 1:size(data_matrix, 2)
                ...
                [svd_final_aoa, ~, ~, ~, ~] = MSSA_SVD_findOptimalWandK_filtering_onetime_max_magnitude(X, Nr, true_doa, 2, upper, 1, 's', 1);
