...
function [signal_filtered] = filter_signal(signal, true_doa, Nr, doa_threshold, d, lambda)
    ...
    for client_idx = 1:size(signal_filtered, 1)
        ...
        for frame_idx = 1:size(signal_filtered, 2)
            ...
            frame_good_samples = find_frame_good_samples_hankel(client_true_doa, client_samples, Nr, doa_threshold, client_idx, false, d, lambda);
...
function [sample_good_idx] = find_frame_good_samples_hankel(true_doa, frame_samples, Nr, doa_threshold, client_idx, plot_errors, d, lambda)
    ...
    for sample_idx = 1:size(frame_errors, 1)
        ...
        [mf_doa,~, ~] = aoa(X, Nr, 1, d, lambda);

        % frame_errors(sample_idx, 1) = abs(true_doa - svd_final_doa);
        frame_errors(sample_idx, 1) = abs(true_doa - mf_doa);
    end
...
function [doa_rmse] = evaluate_hankel(doa_true_angles, test_signal_sim, Nr, N, upper, d, lambda)
    ...
    for doa_idx = 1:length(doa_true_angles) 
        ...
        for frame_idx = 1:N
            ...
            for sample_idx = 1:size(data_matrix, 2)
                ...
                [svd_final_aoa, ~, ~, ~, ~] = MSSA_SVD_findOptimalWandK_filtering_onetime_max_magnitude(X, Nr, true_doa, 2, upper, 1, 's', 1, d, lambda);
